...
func (loc *Locale) Validation(propName string, propValue interface{}) interface{} {
	pm := map[string]func() interface{}{
		...
		"locales": func() interface{} {
			value := loc.Locales
			if lang, valid := propValue.([]SelectOption); valid && len(lang) > 0 {
				value = lang
			}
			if len(value) == 0 {
		...
		"tag_keys": func() interface{} {
			value := loc.TagKeys
			if key, valid := propValue.([]SelectOption); valid && len(key) > 0 {
				value = key
			}
			return value
		},
...
func (loc *Locale) SetProperty(propName string, propValue interface{}) interface{} {
	pm := map[string]func() interface{}{
		"locales": func() interface{} {
			loc.Locales = loc.Validation(propName, propValue).([]SelectOption)
			return loc.Locales
		},
		"tag_keys": func() interface{} {
			loc.TagKeys = loc.Validation(propName, propValue).([]SelectOption)
			return loc.TagKeys
		},
...
func (loc *Locale) response(evt ResponseEvent) (re ResponseEvent) {
	locEvt := ResponseEvent{
...
func (loc *Locale) getComponent(name string, data ut.IM) (res string, err error) {
	ccSel := func(options []SelectOption) *Select {
		sel := &Select{
			BaseComponent: BaseComponent{
	...
	ccInp := func(label, placeholder, value string) *Input {
		inp := &Input{
			BaseComponent: BaseComponent{
	...
	ccTbl := func(rowKey string, rows []ut.IM, fields []TableField) *Table {
		tbl := &Table{
			BaseComponent: BaseComponent{
				Id: loc.Id + "_" + name, Name: name,
				EventURL:     loc.EventURL,
	...
	ccMap := map[string]func() ClientComponent{
		"locales": func() ClientComponent {
			return ccSel(loc.Locales)
		},
		"tag_keys": func() ClientComponent {
			keys := loc.TagKeys
			if ut.ToString(loc.Data["locales"], "") != "default" {
				keys = append(keys, SelectOption{Value: "missing", Text: "missing"})
			}
			return ccSel(keys)
		},
		...
		"values": func() ClientComponent {
			...
			fields := []TableField{
				...
				{Name: "key", FieldType: TableFieldTypeString, Label: loc.msg("locale_key")},
			}
			if lang == "default" {
				fields = append(fields, TableField{
					Name: "default", FieldType: TableFieldTypeString, Label: loc.msg("locale_value")})
			} else {
				fields = append(fields,
					TableField{Column: &TableColumn{
						Id:     "value",
...
var testLocaleResponse func(evt ResponseEvent) (re ResponseEvent) = func(evt ResponseEvent) (re ResponseEvent) {
	...
	case LocalesEventUndo:
		...
		evt.Trigger.SetProperty("locales", []SelectOption{
