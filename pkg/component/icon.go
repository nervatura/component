...
func (ico *Icon) Validation(propName string, propValue interface{}) interface{} {
	...
	if ico.BaseComponent.GetProperty(propName) != nil {
		return ico.BaseComponent.Validation(propName, propValue)
	}
	return propValue
}
...
func (ico *Icon) SetProperty(propName string, propValue interface{}) interface{} {
	pm := map[string]func() interface{}{
		"value": func() interface{} {
			ico.Value = ico.Validation(propName, propValue).(string)
			return ico.Value
		},
		"width": func() interface{} {
			ico.Width = ico.Validation(propName, propValue).(float64)
			return ico.Width
		},
		"height": func() interface{} {
			ico.Height = ico.Validation(propName, propValue).(float64)
			return ico.Height
		},
		"color": func() interface{} {
			ico.Color = ut.ToString(propValue, "")
			return ico.Color
		},
	}
	if _, found := pm[propName]; found {
...
func (ico *Icon) OnRequest(te TriggerEvent) (re ResponseEvent) {
	evt := ResponseEvent{
		Trigger:     ico,
		TriggerName: ico.Name,
		Name:        IconEventClick,
		Value:       ico.Value,
	}
	if ico.OnResponse != nil {
		return ico.OnResponse(evt)
	}
	return evt
}
...
*/
func (ico *Icon) Render() (html template.HTML, err error) {
	ico.InitProps(ico)
	idata := iconMap[ico.Value]

	funcMap := map[string]any{
		"styleMap": func() bool {
			return len(ico.Style) > 0
		},
		"customClass": func() string {
			return strings.Join(ico.Class, " ")
		},
		"viewBox": func() string {
			return idata.ViewBox
		},
		"iPath": func() string {
			return idata.Path
		},
	...
	{{ if ne .EventURL "" }} class="link {{ customClass }}" hx-post="{{ .EventURL }}" hx-target="{{ .Target }}" {{ if ne .Sync "none" }} hx-sync="{{ .Sync }}"{{ end }} hx-swap="{{ .Swap }}"{{ end }}
	{{ if ne .Indicator "none" }} hx-indicator="#{{ .Indicator }}"{{ end }}
	{{ if styleMap }} style="{{ range $key, $value := .Style }}{{ $key }}:{{ $value }};{{ end }}"{{ end }}
	><g{{ if ne .Color "" }} fill="{{ .Color }}"{{ end }}><path d="{{ iPath }}"></path></g>
	</svg>`

	if html, err = ut.TemplateBuilder("icon", tpl, funcMap, ico); err == nil && ico.EventURL != "" {
