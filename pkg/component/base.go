...
much quicker.
  ...
  - The development of a client-side application and component takes place in a very complex ecosystem. An average
    node_modules size can be hundreds of MB and contain hundreds or even over a thousand different packages. Each of
    these also means external dependencies of varying quality and reliability, which is also a big security risk. In
    addition, the constant updating and tracking of these different packages and the dozens of frameworks and technologies
    based on them requires a lot of resources. Server-side components have little or no external dependencies. The
    components can be easily created within the technical capabilities of a specific server-side language. Their maintenance
    needs are limited to their actual code, which is very small and much safer due to the lack of large external dependencies.

  - The language of client-side components is basically Javascript, but most server-side languages are much more efficient
    and safer programming languages. JavaScript is originally an add-on to html code and browsers, which was originally created
    to increase the efficiency of the user interface and not to develop the codebase of complex programs. During development,
    it is possible to partially replace it with, for example, the Typescript language, but this also means additional dependencies
    and an increase in the complexity of the development ecosystem, the end result of which will still be a Javascript codebase.
    This practically means that a code written in a programming language is translated into the code of another language, and
    the content to be displayed is created during its execution. In the process, there are many intermediate steps, used resources,
    error possibilities, security risks and uncertainties. With the server-side components, it is possible to simply write the
    program code in an easy-to-use and safe language, the end result of which is the html content to be displayed.

  - Client-side components usually communicate with the server using a JSON-based REST API and receive the data to be
    displayed. This also means that the data retrieval must adapt to the data structure of the REST API. In other words, the database
